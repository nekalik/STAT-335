...
```{R}
trend = lm(arr_del15~date, data = summary_delays)
summary(trend)
tsplot(summary_delays$date, summary_delays$arr_del15)
...
test = lm(log(arr_del15)~date, data = summary_delays)
summary(test)

tsplot(summary_delays$date, log(summary_delays$arr_del15))
abline(test,col="red")
...
```
```{R}
#build a regression model for relationship bw delay and total counts
trend = lm(arr_del15~date+ arr_flights, data = summary_delays)
summary(trend)
tsplot(summary_delays$date, summary_delays$arr_del15)
abline(trend,col="red")
par(mfrow=c(2,2)) 
...
plot(summary_delays$arr_flights, summary_delays$arr_del15)
```
...
#differencing
tsplot(diff(summary_delays$arr_del15))
acf1(diff(summary_delays$arr_del15),30)
