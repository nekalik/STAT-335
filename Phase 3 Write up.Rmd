...
We used our previous discoveries to guide us in model fitting. Since there's a linear trend, we differenced the data. In addition, we attempted to build some models with log(x) due to possible heteroscedasticity. From the time series plot of number of delays vs time, there seems to be seasonality in the data, so we tried fitting a SARIMA model. 

After examining the acf and pacf of x and log(x), we came up with the following models (shown in the table) where the standarized residuals are white noise like, the acf values are within the confidence bands, the Q-Q plot seems to be straight, and the p-values for the Ljung-Box statistic are above 0.05. Since the AIC, AICc, and BIC are approximately equal for each model, we only recorded the AIC in the table. 

A SARIMA model is not included because when we tried to do a $ARIMA_{x}(1,1,1) \times (0,1,1)_{12}$ model, the code failed. The error could be caused by the lack of data, since after removing the data after COVID, the data set is restricted to 36 points (2017-2020).
...
$AR_x(1)$               &           & $22.62471$   &  &  \\
$MA_x(2)$               &           & $22.597$     &  &  \\
$ARIMA_x(0,1,2)$        &           & $22.73819$   &  &  \\
$ARIMA_{log(x)}(1,1,0)$ &           & $-0.1939758$ &  &  \\
$ARIMA_{log(x)}(1,1,1)$ & example   & $-0.1974$    &  & 
\end{tabular}
\end{table}
